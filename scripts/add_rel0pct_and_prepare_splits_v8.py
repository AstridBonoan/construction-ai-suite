"""Compute will_delay_rel0pct label, update aggregated CSV, produce diagnostics and splits.

Outputs:
- updates `data_splits/project_level_aggregated_v8_relaxed.csv` (adds `will_delay_rel0pct`)
- diagnostics: `data_splits/v8/diagnostics_rel0pct.txt`
- splits: `data_splits/v8/X_train_rel0pct.csv`, `X_test_rel0pct.csv`, `y_train_rel0pct.csv`, `y_test_rel0pct.csv`
- filtered splits excluding low-confidence: `*_rel0pct_filtered.csv`
...
OUT_DIAG = ROOT / 'data_splits' / 'v8' / 'diagnostics_rel0pct.txt'
...
def main():
    ...
    # Compute will_delay_rel0pct: relative delay > 0, only when planned_duration_days > 0
    df['will_delay_rel0pct'] = 0
    mask = (df['planned_duration_days'] > 0) & df['elapsed_days'].notna()
    rel = (df['elapsed_days'] - df['planned_duration_days']) / df['planned_duration_days']
    df.loc[mask & (rel > 0), 'will_delay_rel0pct'] = 1
    ...
    # Save updated aggregated CSV
    df.to_csv(AGG, index=False)

    # Diagnostics
    total = len(df)
    positives = int(df['will_delay_rel0pct'].sum())
    ...
    diag = {
        ...
        'will_delay_rel0pct_positives': int(positives),
        'will_delay_rel0pct_negatives': int(negatives),
    ...
    # Prepare splits for training using rows where will_delay_rel0pct is defined (computed as above)
    computable = df[df['planned_duration_days'] > 0].copy()
    # require elapsed present
    computable = computable[computable['elapsed_days'].notna()].copy()

    # Use will_delay_rel0pct as label
    y = computable['will_delay_rel0pct'].astype(int)
    X = computable.drop(columns=['will_delay_rel0pct', 'will_delay', 'will_delay_abs30'], errors=True)
    ...
    # Save splits
    X_train.to_csv(OUT_DIR / 'X_train_rel0pct.csv', index=False)
    X_test.to_csv(OUT_DIR / 'X_test_rel0pct.csv', index=False)
    y_train.to_frame('will_delay_rel0pct').to_csv(OUT_DIR / 'y_train_rel0pct.csv', index=False)
    y_test.to_frame('will_delay_rel0pct').to_csv(OUT_DIR / 'y_test_rel0pct.csv', index=False)

    # Filtered splits excluding low-confidence
    filtered = computable[computable['label_confidence'].isin(['high', 'medium'])].copy()
    if len(filtered) > 0:
        yf = filtered['will_delay_rel0pct'].astype(int)
        Xf = filtered.drop(columns=['will_delay_rel0pct', 'will_delay', 'will_delay_abs30'], errors=True)
        ...
        Xf_train.to_csv(OUT_DIR / 'X_train_rel0pct_filtered.csv', index=False)
        Xf_test.to_csv(OUT_DIR / 'X_test_rel0pct_filtered.csv', index=False)
        yf_train.to_frame('will_delay_rel0pct').to_csv(OUT_DIR / 'y_train_rel0pct_filtered.csv', index=False)
        yf_test.to_frame('will_delay_rel0pct').to_csv(OUT_DIR / 'y_test_rel0pct_filtered.csv', index=False)
    ...
    # Save diag (json already written)
    print('Computed will_delay_rel0pct, wrote diagnostics and splits.')
