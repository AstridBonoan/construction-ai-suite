...
def main():
    ...
    rel_slippage = (df["elapsed_days"] - df["planned_duration_days"]) / df[
        "planned_duration_days"
    ]
    ...
    X = computable.drop(
        columns=["will_delay_rel1pct", "will_delay", "will_delay_abs30"], errors=True
    )
    ...
    y_train.to_frame("will_delay_rel1pct").to_csv(
        OUT_DIR / "y_train_rel1pct.csv", index=False
    )
    y_test.to_frame("will_delay_rel1pct").to_csv(
        OUT_DIR / "y_test_rel1pct.csv", index=False
    )
    ...
    if len(filtered) > 0:
        ...
        Xf = filtered.drop(
            columns=["will_delay_rel1pct", "will_delay", "will_delay_abs30"],
            errors=True,
        )
        # drop deny list
        ...
        yf_train.to_frame("will_delay_rel1pct").to_csv(
            OUT_DIR / "y_train_rel1pct_filtered.csv", index=False
        )
        yf_test.to_frame("will_delay_rel1pct").to_csv(
            OUT_DIR / "y_test_rel1pct_filtered.csv", index=False
        )
    ...
    print(
        "Updated aggregated CSV with will_delay_rel1pct and wrote diagnostics/splits."
    )
