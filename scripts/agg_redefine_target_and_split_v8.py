...
def aggregate_to_project(df, parsed_map, yearonly_map, group_key="project_id"):
    ...
    for project_id, sub in grp:
                ...
                if pc in sub:
                    ...
                    if require_full and full_flag in sub:
                        ...
                        series = pd.to_datetime(
                            sub.loc[mask, pc].dropna(), errors="coerce"
                        )
                    else:
                        if (
                            allow_year
                            and year_flag in sub
                            and sub[year_flag].astype(bool).any()
                        ):
        ...
        planned_candidates = list(parsed_map.get("planned_start", [])) + list(
            parsed_map.get("planned_end", [])
        )
        planned_dates = collect_series(
            planned_candidates, require_full=True, allow_year=False
        )
        ...
        actual_start_full = collect_series(
            parsed_map.get("actual_start", []), require_full=True, allow_year=False
        )
        actual_start_any = collect_series(
            parsed_map.get("actual_start", []), require_full=False, allow_year=True
        )
        actual_end_full = collect_series(
            parsed_map.get("actual_end", []), require_full=True, allow_year=False
        )
        actual_end_any = collect_series(
            parsed_map.get("actual_end", []), require_full=False, allow_year=True
        )

        entry["_actual_start_full"] = (
            actual_start_full.min() if actual_start_full is not None else pd.NaT
        )
        entry["_actual_start_any"] = (
            actual_start_any.min() if actual_start_any is not None else pd.NaT
        )
        entry["_actual_end_full"] = (
            actual_end_full.max() if actual_end_full is not None else pd.NaT
        )
        entry["_actual_end_any"] = (
            actual_end_any.max() if actual_end_any is not None else pd.NaT
        )
    ...
    median_planned = (
        float(np.median(planned_durations)) if len(planned_durations) > 0 else 365.0
    )
    ...
    for entry in temp:
        ...
        try:
            planned_end_orig_cols = [
                pc.replace("_parsed__", "") for pc in parsed_map.get("planned_end", [])
            ]
        ...
        has_planned_full = (not pd.isna(entry.get("_planned_start"))) and (
            not pd.isna(entry.get("_planned_end"))
        )
        ...
        try:
            ...
            actual_end_orig_cols = [
                pc.replace("_parsed__", "") for pc in parsed_map.get("actual_end", [])
            ]
        ...
        # impute using median planned duration across projects
            ...
            pd.isna(actual_start) and not pd.isna(actual_end) and (not has_planned_full)
        ):
        ...
        out["actual_start"] = (
            pd.NaT if pd.isna(actual_start) else pd.Timestamp(actual_start)
        )
        ...
        out["planned_duration_days"] = (
            float(planned_duration) if not pd.isna(planned_duration) else np.nan
        )
        out["elapsed_days"] = (
            float(elapsed_days) if not pd.isna(elapsed_days) else np.nan
        )
        ...
        if pd.isna(out["planned_duration_days"]):
            ...
            if pd.isna(out["elapsed_days"]):
                ...
                if out["planned_duration_days"] > 0:
                    out["schedule_slippage_pct"] = (
                        out["delay_days"] / out["planned_duration_days"]
                    )
                else:
                    out["schedule_slippage_pct"] = np.nan
        ...
        parse_fallbacks.append(
            {
                ...
                "used": entry.get("_used", {}),
                "year_only_parse": entry.get("_year_only_used", []),
                "actual_start_imputed": bool(actual_start_imputed),
            }
        )
...
def main():
    ...
    parser.add_argument(
        "--stratify",
        action="store_true",
        help="Attempt stratified split if positives exist",
    )
    ...
    candidate_map = {
        k: [c for c in v if c in df.columns] for k, v in candidate_map.items()
    }

    # Parse date candidates and create parsed columns
    df_parsed, parsed_map, yearonly_map, parsed_orig = coerce_and_parse_dates(
        df, candidate_map
    )

    # Aggregate
    agg, parse_fallbacks = aggregate_to_project(
        df_parsed, parsed_map, yearonly_map, group_key="project_id"
    )
    ...
    agg["will_delay"] = (
        ((agg["schedule_slippage_pct"] > threshold) | (agg["delay_days"] > 0))
        .fillna(False)
        .astype(int)
    )
    # absolute 30-day delay
    agg["will_delay_abs30"] = (
        (pd.to_numeric(agg.get("delay_days", pd.Series()), errors="coerce") > 30)
        .fillna(False)
        .astype(int)
    )
    # relative 5% slippage over planned, only when planned_duration_days > 14
    planned_pos = pd.to_numeric(
        agg.get("planned_duration_days", pd.Series()), errors="coerce"
    )
    ...
    print(
        f"Project-level target: total={total} will_delay_pos={positives} will_delay_abs30_pos={abs30_pos} will_delay_rel5pct_pos={rel5_pos} negatives={negatives}"
    )
    ...
    ruleA_path = os.path.join(
        os.path.dirname(args.output_agg), "project_level_aggregated_v8_ruleA.csv"
    )
    ...
    imputed_count = int(
        pd.to_numeric(agg.get("actual_start_imputed", pd.Series(0)), errors="coerce")
        .fillna(0)
        .sum()
    )
    ...
    try:
        ...
        counts_by_rule = {}
    computable_count = (
        int(agg["delay_days"].notna().sum()) if "delay_days" in agg.columns else 0
    )
    ...
    try:
        ...
        print(
            "Stratified split failed or not possible, falling back to random split:", e
        )
        X_train, X_test, y_train, y_test = train_test_split(
    ...
    y_train.to_csv(
        os.path.join(splits_dir, "y_train.csv"), index=False, header=["will_delay"]
    )
    y_test.to_csv(
        os.path.join(splits_dir, "y_test.csv"), index=False, header=["will_delay"]
    )
    ...
    y_train_abs30.to_csv(
        os.path.join(splits_dir, "y_train_abs30.csv"),
        index=False,
        header=["will_delay_abs30"],
    )
    y_test_abs30.to_csv(
        os.path.join(splits_dir, "y_test_abs30.csv"),
        index=False,
        header=["will_delay_abs30"],
    )
    ...
    y_train_rel5.to_csv(
        os.path.join(splits_dir, "y_train_rel5pct.csv"),
        index=False,
        header=["will_delay_rel5pct"],
    )
    y_test_rel5.to_csv(
        os.path.join(splits_dir, "y_test_rel5pct.csv"),
        index=False,
        header=["will_delay_rel5pct"],
    )
