...
def main():
    ...
    imputed_count = 0
    imputation_rules = df.get('imputation_rule', pd.Series(['none'] * len(df))).fillna('none')
    actual_start_imputed = df.get('actual_start_imputed', pd.Series([False] * len(df))).astype(bool).fillna(False)
    label_confidence = df.get('label_confidence', pd.Series(['low'] * len(df))).fillna('low')
    ...
    for idx, row in df.iterrows():
        ...
        if (pd.isna(actual_start_dt) or pd.isna(df.at[idx, 'actual_start_dt'])) and (not pd.isna(actual_end_dt)):
            ...
            if np.isnan(median_d):
                median_d = global_median
            if not np.isnan(median_d):
                new_actual_start = pd.to_datetime(actual_end_dt) - pd.Timedelta(days=int(round(median_d)))
    ...
    mask = (df['planned_duration_days'].notna()) & (df['planned_duration_days'] > 0) & (df['elapsed_days'].notna())
    df.loc[mask, 'delay_days'] = df.loc[mask, 'elapsed_days'] - df.loc[mask, 'planned_duration_days']
    df.loc[mask & (df['planned_duration_days'] > 0), 'schedule_slippage_pct'] = df.loc[mask, 'delay_days'] / df.loc[mask, 'planned_duration_days']
    ...
    X = computable.drop(columns=['will_delay_ruleB', 'will_delay', 'will_delay_abs30', 'will_delay_rel1pct', 'will_delay_rel0pct'], errors=True)
    ...
    y_train.to_frame('will_delay_ruleB').to_csv(OUT_DIR / 'y_train_ruleB.csv', index=False)
    y_test.to_frame('will_delay_ruleB').to_csv(OUT_DIR / 'y_test_ruleB.csv', index=False)
    ...
    if len(filtered) > 0:
        ...
        Xf = filtered.drop(columns=['will_delay_ruleB', 'will_delay', 'will_delay_abs30', 'will_delay_rel1pct', 'will_delay_rel0pct'], errors=True)
        ...
        yf_train.to_frame('will_delay_ruleB').to_csv(OUT_DIR / 'y_train_ruleB_filtered.csv', index=False)
        yf_test.to_frame('will_delay_ruleB').to_csv(OUT_DIR / 'y_test_ruleB_filtered.csv', index=False)
