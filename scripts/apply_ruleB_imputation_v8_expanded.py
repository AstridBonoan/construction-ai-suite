...
def infer_types(df, project_type_col):
    known_map = {
        'high school': 'High School', 'hs': 'High School', 'highschool': 'High School',
        'elementary': 'Elementary', 'elem': 'Elementary', 'primary': 'Elementary',
        'middle': 'Middle', 'middleschool': 'Middle', 'ms': 'Middle',
        'hospital': 'Hospital', 'medical': 'Hospital',
        'bridge': 'Bridge',
        'park': 'Park', 'playground': 'Park',
        'residential': 'Residential', 'housing': 'Residential', 'apartment': 'Residential', 'apt': 'Residential',
        'library': 'Library',
        'road': 'Road', 'roadway': 'Road', 'street': 'Road', 'st': 'Road',
        'sewer': 'Utility', 'water': 'Utility',
        'police': 'Police', 'fire': 'Fire',
        'hotel': 'Hotel', 'office': 'Office', 'commercial': 'Commercial', 'retail': 'Commercial',
        'stadium': 'Stadium', 'airport': 'Airport',
        'school': 'School'
    }
    known_types = sorted(list(set(known_map.values())))

    text_cols = [c for c in df.columns if any(k in c.lower() for k in ('title', 'desc', 'description', 'name'))]
    ...
    for idx, row in df.iterrows():
        ...
        if txt:
            ...
            for k, v in known_map.items():
                if (' ' + k + ' ') in (' ' + txt + ' ') or txt.startswith(k) or txt.endswith(k) or (' ' + k + ',') in txt:
                    chosen = v
                    break
            ...
            # fuzzy match tokens
            if not chosen:
                tokens = [t for t in txt.replace('/', ' ').replace('-', ' ').split() if len(t) > 2]
                for t in tokens:
                    cm = get_close_matches(t, list(known_map.keys()) + known_types, n=1, cutoff=0.85)
                    if cm:
                        cand = cm[0]
                        # map back if key
...
def main():
    ...
    out_relaxed_expanded = 'data_splits/project_level_aggregated_v8_relaxed_with_types_expanded.csv'
    ...
    numeric_cols = ['planned_duration_days', 'elapsed_days', 'delay_days', 'schedule_slippage_pct']
    ...
    missing_before = df[project_type_col].isna() | (df[project_type_col].astype(str).str.strip() == '')
    ...
    if 'planned_duration_days' in df_inf.columns:
        df_inf['planned_duration_days'] = pd.to_numeric(df_inf['planned_duration_days'], errors='coerce')
    else:
        df_inf['planned_duration_days'] = pd.NA

    pos_pd = df_inf[df_inf['planned_duration_days'] > 0]
    per_type_medians = pos_pd.groupby(project_type_col)['planned_duration_days'].median()
    ...
    for idx, row in df_inf.iterrows():
        ...
        try:
            ...
            pdur_n = None
        if planned_start_dt is not None and pdur_n is not None and not math.isnan(pdur_n):
    ...
    counts_by_imputation = df_inf['imputation_rule'].value_counts(dropna=False).to_dict() if 'imputation_rule' in df_inf.columns else {}
    counts_by_confidence = df_inf['label_confidence'].value_counts(dropna=False).to_dict() if 'label_confidence' in df_inf.columns else {}
