...
def aggregate_project_level(
    ...
    logger.info(
        "Parsing dates: start=%s, planned=%s, actual=%s",
        start_col,
        planned_col,
        actual_col,
    )
    start_dt = (
        clean_parse_dates(df[start_col])
        if start_col in df.columns
        else pd.Series(pd.NaT, index=df.index)
    )
    planned_dt = (
        clean_parse_dates(df[planned_col])
        if planned_col in df.columns
        else pd.Series(pd.NaT, index=df.index)
    )
    actual_dt = (
        clean_parse_dates(df[actual_col])
...
def summarize_and_save(agg: pd.DataFrame, output_csv: Path, summary_csv: Path) -> None:
    ...
    if counts[">0.0"] == 0:
        logger.error(
            "No projects with schedule_slippage_pct > 0 found. Aborting for inspection."
        )
        raise SystemExit(2)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Clean planned dates and aggregate schedule slippage"
    )
    p.add_argument(
        "--input",
        "-i",
        required=True,
        help="Row-level cleaned CSV (with planned candidate column)",
    )
    p.add_argument(
        "--planned-col",
        default="Project Phase Planned End Date",
        help="Planned end date candidate column",
    )
    p.add_argument(
        "--actual-col",
        default="Project Phase Actual End Date",
        help="Actual end date column",
    )
    p.add_argument(
        "--start-col",
        default="Project Phase Actual Start Date",
        help="Actual start date column",
    )
    p.add_argument(
        "--output",
        "-o",
        default="data_splits/project_level_aggregated_with_slippage.csv",
        help="Output CSV for aggregated projects",
    )
    p.add_argument(
        "--summary",
        default="data_splits/schedule_slippage_summary.csv",
        help="CSV summary of slippage stats",
    )
    return p.parse_args()
...
def main():
    ...
    for c in (args.planned_col, args.actual_col, args.start_col):
        if c not in df.columns:
            logger.warning(
                "Column %s not found in input; proceeding but values will be NaT where missing",
                c,
            )

    agg = aggregate_project_level(
        df, proj_col, args.start_col, args.planned_col, args.actual_col
    )

    # Standardize planned_end to ISO where present
    if "planned_end" in agg.columns:
        agg["planned_end_iso"] = pd.to_datetime(
            agg["planned_end"], errors="coerce"
