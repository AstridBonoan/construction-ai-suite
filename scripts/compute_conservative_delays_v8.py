...
SUMMARY_OUT = os.path.join('data_splits', 'v8', 'project_level_aggregated_v8_conservative_summary.txt')
...
def used_contains_full_date(used_list):
    ...
    for token in used_list:
        ...
        if 'Date' in t or 'Project Phase' in t or 'Filed' in t or 'Permit' in t or 'Complt' in t:
            return True
    return False
...
def main():
    ...
    for i, r in agg.iterrows():
        ...
        try:
            pstart = pd.to_datetime(planned_start, errors='coerce') if planned_start else pd.NaT
            pend = pd.to_datetime(planned_end, errors='coerce') if planned_end else pd.NaT
            astart = pd.to_datetime(actual_start, errors='coerce') if actual_start else pd.NaT
        ...
        if cons_planned_duration is not pd.NA and cons_elapsed is not pd.NA and pd.notna(cons_planned_duration):
            # both numeric
            try:
        ...
        if pd.notna(cons_planned_duration) and pd.notna(cons_elapsed) and pd.notna(cons_delay):
            can_compute = True
    ...
    lines = []
    lines.append(f"Conservative delay computation run: {datetime.utcnow().isoformat()}Z")
    lines.append(f"Input aggregated projects: {total}")
    lines.append(f"Projects with computable delay (all four full-date parses present): {computable}")
