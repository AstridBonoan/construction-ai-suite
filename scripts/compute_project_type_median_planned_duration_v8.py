...
OUT_CSV = ROOT / 'data_splits' / 'v8' / 'project_type_median_planned_duration_summary.csv'
OUT_TXT = ROOT / 'data_splits' / 'v8' / 'project_type_median_planned_duration_summary.txt'
...
def main():
    ...
    ruleb['planned_duration_days'] = pd.to_numeric(ruleb.get('planned_duration_days'), errors='coerce')
    ...
    if RELAXED.exists():
        ...
        relaxed['planned_duration_days'] = pd.to_numeric(relaxed.get('planned_duration_days'), errors='coerce')
        # cap >1825
        relaxed.loc[relaxed['planned_duration_days'] > 1825, 'planned_duration_days'] = np.nan
        if pt_col and pt_col in relaxed.columns:
            ...
            for k, g in groups:
                ...
                medians[k if k != '' else None] = float(vals.median()) if len(vals) > 0 else np.nan
        ...
        if pt_col and pt_col in ruleb.columns:
            groups = ruleb[ruleb.get('imputation_rule', '') != 'RuleB'].groupby(ruleb[pt_col].fillna(''))
            for k, g in groups:
                vals = pd.to_numeric(g['planned_duration_days'], errors='coerce').dropna()
                vals = vals[(vals > 0) & (vals <= 1825)]
                medians[k if k != '' else None] = float(vals.median()) if len(vals) > 0 else np.nan
        global_vals = pd.to_numeric(ruleb[ruleb.get('imputation_rule', '') != 'RuleB']['planned_duration_days'], errors='coerce').dropna()
        global_vals = global_vals[(global_vals > 0) & (global_vals <= 1825)]
        global_median = float(global_vals.median()) if len(global_vals) > 0 else np.nan
    ...
    for t in types:
        ...
        if RELAXED.exists():
            relaxed = pd.read_csv(RELAXED, dtype=str)
            relaxed['planned_duration_days'] = pd.to_numeric(relaxed.get('planned_duration_days'), errors='coerce')
            merged = ruleb.merge(relaxed[[id_col, 'planned_duration_days']], how='left', on=id_col, suffixes=('', '_orig'))
            orig = merged['planned_duration_days_orig']
            imp = merged['planned_duration_days']
            merged_pt = merged[pt_col].fillna('') if pt_col and pt_col in merged.columns else pd.Series([''] * len(merged), index=merged.index)
            mask = (merged_pt == (t if t is not None else '')) & ((orig.isna()) | (orig <= 0)) & (imp.notna() & (imp > 0))
            imputed_count = int(mask.sum())
        else:
            # conservative: count rows with imputation_rule == RuleB for this type
            imputed_count = int(((ruleb_pt == (t if t is not None else '')) & (ruleb.get('imputation_rule', '') == 'RuleB')).sum())

        rows.append({'project_type': (t if t != '' else '<unknown>'),
                     'median_planned_duration_days': float(median) if not np.isnan(median) else np.nan,
                     'count_projects': count_projects,
                     'count_imputed_planned_duration': imputed_count})

    out_df = pd.DataFrame(rows).sort_values(by='count_imputed_planned_duration', ascending=False)
    OUT_CSV.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(OUT_CSV, index=False)

    # Overall diagnostics
    total_projects = len(ruleb)
    total_imputed = int(((ruleb.get('imputation_rule', '') == 'RuleB') | ((pd.to_numeric(ruleb.get('planned_duration_days'), errors='coerce') > 0) & True)).sum())
    ...
    with open(OUT_TXT, 'w', encoding='utf-8') as f:
        ...
        f.write(f'Total imputed (imputation_rule==RuleB): {int((ruleb.get("imputation_rule","")=="RuleB").sum())}\n')
        f.write('\nPer-project-type medians and counts (see CSV):\n')
        for _, r in out_df.iterrows():
            f.write(f" - {r['project_type']}: median={r['median_planned_duration_days']}, count={int(r['count_projects'])}, imputed={int(r['count_imputed_planned_duration'])}\n")
