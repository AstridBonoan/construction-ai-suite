...
OUT_SUM = (
    ROOT / "data_splits" / "v8" / "project_level_aggregated_v8_relaxed_summary.txt"
)
...
def load_fallbacks(path: Path):
    ...
    if isinstance(data, list):
        return {
            rec.get("project_id") or rec.get("project") or str(i): rec
...
def main():
    ...
    for ix, row in df.iterrows():
        ...
        can_compute = bool(
            planned_start_full
            and planned_end_full
            and actual_start_full
            and (
                actual_end_full_or_year
                or (not pd.isna(row.get("actual_end")) and row.get("actual_end") != "")
            )
        )
