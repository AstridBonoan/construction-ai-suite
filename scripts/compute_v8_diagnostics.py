...
planned_non_na = int(
    pd.to_numeric(agg.get("planned_duration_days", pd.Series()), errors="coerce")
    .notna()
    .sum()
)
elapsed_non_na = int(
    pd.to_numeric(agg.get("elapsed_days", pd.Series()), errors="coerce").notna().sum()
)
delay_non_na = int(
    pd.to_numeric(agg.get("delay_days", pd.Series()), errors="coerce").notna().sum()
)

# positives
delay_pos = int(
    (pd.to_numeric(agg.get("delay_days", pd.Series()), errors="coerce") > 0).sum()
)
slip_pos = int(
    (
        pd.to_numeric(agg.get("schedule_slippage_pct", pd.Series()), errors="coerce")
        > 0
    ).sum()
)
...
projects_using_any = sum(
    1 for r in fallbacks if any(len(v) > 0 for v in r.get("used", {}).values())
)
projects_failed_all = sum(
    1
    for r in fallbacks
    if not any(len(v) > 0 for v in r.get("used", {}).values())
    and not r.get("year_only_parse")
)
