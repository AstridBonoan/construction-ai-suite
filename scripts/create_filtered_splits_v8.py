...
def main():
    ...
    total = len(df)
    low_count = (
        int((df["label_confidence"].fillna("low") == "low").sum())
    ...
    if "will_delay_abs30" in dff.columns:
        ...
        y_train_abs30.to_frame("will_delay_abs30").to_csv(
            OUT_DIR / "y_train_abs30.csv", index=False
        )
        y_test_abs30.to_frame("will_delay_abs30").to_csv(
            OUT_DIR / "y_test_abs30.csv", index=False
        )
