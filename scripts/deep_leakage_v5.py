    ...
    for df_name, X, y in [("train", X_train, y_train), ("test", X_test, y_test)]:
        for c in X.columns:
            ...
            try:
                ...
                if s_num.notna().sum() >= 2:
                    sp = spearmanr(
                        s_num, pd.to_numeric(y, errors="coerce"), nan_policy="omit"
                    ).correlation
                else:
                    sp = np.nan
                    ...
                    if s_num.notna().sum() > 0:
                        ...
                        mi = float(
                            mutual_info_regression(
                                s_num.fillna(0).values.reshape(-1, 1),
                                pd.to_numeric(y, errors="coerce").fillna(0),
                        ...
                        mi = float(
                            mutual_info_regression(
                                pd.factorize(s.fillna(""))[0].reshape(-1, 1),
                                pd.to_numeric(y, errors="coerce").fillna(0),
        ...
        try:
            ...
            if Xtr_train is not None:
                ...
                for idx, fname in enumerate(names):
                    ...
                    # exact
                    try:
                        eq = col_tr.fillna("").astype(str) == y_train.fillna("").astype(
                            str
                        )
                    ...
                    try:
                        pear = float(
                            col_tr.corr(pd.to_numeric(y_train, errors="coerce"))
                        )
                    except Exception:
                        pear = np.nan
                    ...
                    try:
                        ...
                        flags["spearman_train_trans"] = (
                            float(sp) if sp is not None else np.nan
                        )
                    except Exception:
                    ...
                    try:
                        mi = float(
                            mutual_info_regression(
                                col_tr.fillna(0).values.reshape(-1, 1),
                                pd.to_numeric(y_train, errors="coerce").fillna(0),
                                discrete_features=False,
                            )[0]
                        )
                    except Exception:
    ...
    with open(TXT_OUT, "w", encoding="utf8") as fh:
        ...
        for _, r in df_out.iterrows():
            ...
            if (
                r.get("exact_train")
                or r.get("exact_test")
                or r.get("exact_num_train")
                or r.get("exact_num_test")
            ):
                note.append("exact_match")
            try:
                if (
                    abs(float(r.get("pearson_train", 0) or 0)) >= 0.95
                    or abs(float(r.get("pearson_test", 0) or 0)) >= 0.95
                ):
            ...
            try:
                if (
                    abs(float(r.get("spearman_train", 0) or 0)) >= 0.95
                    or abs(float(r.get("spearman_test", 0) or 0)) >= 0.95
                ):
        ...
        if suspicious:
            ...
            fh.write(
                "No single-column exact matches or extremely high single-column associations found.\n"
            )
