...
def find_candidates(df: pd.DataFrame) -> List[str]:
    ...
    for c in df.columns:
        lc = c.lower()
        if any(tok in lc for tok in PLANNED_TOKENS) and not any(
            ex in lc for ex in EXCLUDE_TOKENS
        ):
    ...
    if parsed_count > 0 and actual_cols:
        ...
        for a in actual_cols:
            ...
            if common_mask.any():
                ...
                precede_frac = float(
                    (parsed[common_mask] <= a_parsed[common_mask]).mean()
                )
                # median lead (actual - planned) in days
                lead_days = (
                    a_parsed[common_mask] - parsed[common_mask]
                ).dt.days.median()
                precedence_info[a] = {
                    "precede_frac": precede_frac,
                    "median_lead_days": int(lead_days) if pd.notna(lead_days) else None,
                }
...
def find_actual_end_candidates(df: pd.DataFrame) -> List[str]:
    ...
    if not actuals:
        for c in df.columns:
            lc = c.lower()
            if ("actual" in lc or "end" in lc) and not any(
                ex in lc for ex in EXCLUDE_TOKENS
            ):
...
def main():
    p = argparse.ArgumentParser(
        description="Diagnose planned/scheduled end-date candidate columns"
    )
    p.add_argument(
        "--input",
        "-i",
    ...
    logger.info(
        "Found %d actual-end candidate columns: %s",
    ...
    for c in candidates:
        ...
        logger.info(
            "Column: %s — non-null %d (%.2f%%), parsed %d (%.2f%%)\n  parsed range: %s to %s\n  examples: %s\n",
            res["column"],
            res["non_null_count"],
            res["non_null_pct"],
            res["parsed_count"],
            ...
            res["parsed_min"],
            res["parsed_max"],
            json.dumps(res["examples"]),
        )
        if res["precedence_info"]:
            logger.info(
                "  Precedence vs actuals: %s", json.dumps(res["precedence_info"])
            )
    ...
    logger.info(
        "Saved diagnostics to %s", outdir / "planned_date_candidate_diagnostics.csv"
    )
    ...
    if ranked:
        ...
        for col, pscore, frac in ranked:
            logger.info(
                " - %s : parsed_pct=%.2f%%, max_precede_frac=%.2f", col, pscore, frac
            )
        best = ranked[0]
        logger.info(
            "\nBest candidate: %s (parsed_pct=%.2f%%, precede_frac=%.2f)",
            best[0],
            best[1],
            best[2],
        )
