...
patterns = ['slippage', 'schedule', 'target', 'date', 'start', 'end', 'actual', 'planned', 'id', 'project', 'phase']
pattern_matches = []
for c in X.columns:
    low = c.lower()
...
for c in X.columns:
    info = {'feature': c, 'matches': '', 'dtype': str(X[c].dtype), 'n_unique': int(X[c].nunique(dropna=False))}
    low = c.lower()
    matched = [p for p in patterns if p in low]
    ...
    try:
        ...
        try:
            ...
            if not s_num.isna().all() and not y_num.isna().all():
                if (s_num.dropna().reset_index(drop=True).equals(y_num.dropna().reset_index(drop=True))):
                    is_perfect = True
        except Exception:
            pass
    except Exception:
...
candidates = res_df[(res_df['matches'] != '') | (res_df['perfect_predictor']) | (res_df['abs_corr'] >= 0.9)].copy()
...
with open(TXT_OUT, 'w', encoding='utf8') as fh:
    ...
    for _, r in candidates.iterrows():
        fh.write(f"- {r['feature']}: matches={r['matches']}, dtype={r['dtype']}, n_unique={r['n_unique']}, abs_corr={r['abs_corr']}, perfect={r['perfect_predictor']}\n")
    fh.write('\nSummary stats:\n')
    fh.write(res_df.describe(include='all').to_string())

if HAVE_PLOT:
    try:
        num_candidates = [f for f in candidates['feature'].tolist() if pd.api.types.is_numeric_dtype(X[f])]
        if num_candidates:
