...
def main():
    ...
    for _, r in df.iterrows():
        ...
        actual_start_present = not (
            pd.isna(acs_dt)
            or str(acs_raw).strip() == ""
            or str(acs_raw).lower() == "nan"
        )
        actual_end_present = not (
            pd.isna(ace_dt)
            and (str(ace_raw).strip() == "" or str(ace_raw).lower() == "nan")
        )

        # detect year-only vs full-date for actual_end
        actual_end_is_year_only = is_year_only_marker(ace_raw)
        actual_end_is_full_date = (not pd.isna(ace_dt)) and (
            not actual_end_is_year_only
        )
        ...
        if "can_compute_delay" in r.index:
            try:
                can_compute = bool(
                    str(r["can_compute_delay"]).strip().lower()
                    in ("1", "true", "t", "yes")
                )
            except Exception:
                can_compute = False
            ...
            can_compute = (
                planned_window_valid
                and actual_start_present
                and (actual_end_is_full_date or actual_end_is_year_only)
            )
        ...
        if (
            ...
            and actual_end_present
            and (not pd.isna(acs_dt))
            and (not pd.isna(ace_dt))
        ):
