...
def main():
    ...
    merged = ruleb.merge(relaxed[[id_col, 'planned_duration_days']], how='left', on=id_col, suffixes=('_imputed', '_original'))
    merged.rename(columns={'planned_duration_days_imputed': 'imputed_planned_duration_days',
                           'planned_duration_days_original': 'original_planned_duration_days'}, inplace=True)
    ...
    for col in ['actual_start', 'actual_end', 'elapsed_days', 'delay_days', 'schedule_slippage_pct', 'imputation_rule', 'actual_start_imputed', 'label_confidence']:
    ...
    applied = ((orig.isna() | (orig <= 0)) & (imp.notna() & (imp > 0))) | (merged['imputation_rule'] == 'RuleB')

    merged['imputed_planned_applied'] = applied.astype(bool)

    # select summary columns
    summary_cols = [id_col, 'project_type', 'original_planned_duration_days', 'imputed_planned_duration_days',
                    'actual_start', 'actual_end', 'elapsed_days', 'delay_days', 'schedule_slippage_pct',
                    'imputation_rule', 'actual_start_imputed', 'label_confidence', 'imputed_planned_applied']

    summary = merged[[c for c in summary_cols if c in merged.columns]].copy()

    # counts per project_type
    counts_by_pt = summary.groupby('project_type')['imputed_planned_applied'].sum().sort_values(ascending=False).to_dict()

    # diagnostics
    total_projects = len(summary)
    total_imputed_planned = int(summary['imputed_planned_applied'].sum())
    will_delay_pos = int(pd.to_numeric(merged.get('will_delay_ruleB', 0), errors='coerce').fillna(0).astype(int).sum())
    ...
    relaxed_actual_start = relaxed_idxed.reindex(merged[id_col]).get('actual_start') if id_col in relaxed_idxed.columns else pd.Series([None]*len(merged))
    changed = summary[(summary['imputed_planned_applied']) | (merged['actual_start'].fillna('') != relaxed_actual_start.fillna(''))]
    ...
    with open(OUT_TXT, 'w', encoding='utf-8') as f:
        ...
        f.write(f'Projects with imputed planned_duration (cohort median applied): {total_imputed_planned}\n')
