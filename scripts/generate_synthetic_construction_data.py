    ...
    if os.path.exists(SCHEDULES):
        try:
            with open(SCHEDULES, newline='', encoding='utf-8') as f:
                ...
                names = [r.get('Project School Name') or r.get('Project Building Identifier') or r.get('Project Type') for r in reader]
...
def make_material_rows(project_id: str, date_list: list[datetime], rows_per_project: int) -> list[dict]:
    rows = []
...
def make_equipment_rows(project_id: str, date_list: list[datetime], rows_per_project: int) -> list[dict]:
    rows = []
    for _ in range(rows_per_project):
        d = random.choice(date_list)
...
def main():
    ...
    for pid in projects:
        ...
        equipment_rows.extend(make_equipment_rows(pid, date_list, max(1, int(args.rows_per_project / 3))))
    ...
    write_csv(materials_path, ['project_id','date','material','quantity','unit','unit_price_estimated','total_cost_estimated','is_synthetic','source'], materials_rows)
    write_csv(equipment_path, ['project_id','date','equipment','hours_used','hourly_cost_estimated','cost_estimated','maintenance_flag','is_synthetic','source'], equipment_rows)
