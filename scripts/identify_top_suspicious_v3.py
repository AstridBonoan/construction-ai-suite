...
df["is_id_like"] = (
    df["feature"]
    .str.lower()
    .str.contains("id|project|id\b|version|unnamed|date|time|timestamp")
)
...
with open(TXT_OUT, "w") as fh:
    ...
    for _, r in sus.iterrows():
        fh.write(
            f"{r['feature']}: corr={r['corr_with_will_delay']}, abs_corr={r['abs_corr']}, constant={r['is_constant']}, id_like={r['is_id_like']}\n"
        )
