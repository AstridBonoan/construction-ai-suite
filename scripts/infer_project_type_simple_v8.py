...
KEYWORD_MAP = [
    (['high school', 'hs', 'school', 'elementary', 'middle school', 'primary'], 'School'),
...
def main():
    ...
    # detect columns
    id_col = detect_col(df, ['project_id', 'Project ID', 'ID', 'projectId']) or df.columns[0]
    pt_col = detect_col(df, ['project_type', 'Project Type', 'Project Type '])
    name_col = detect_col(df, ['Project School Name', 'Title', 'Project Description', 'project_name', 'project']) or id_col
    ...
    still_missing = int(((df[pt_col].isna()) | (df[pt_col].astype(str).str.strip() == '')).sum())
    counts_by_rule = df.get('imputation_rule', pd.Series()).fillna('none').value_counts().to_dict()
    counts_by_conf = df.get('label_confidence', pd.Series()).fillna('low').value_counts().to_dict()
