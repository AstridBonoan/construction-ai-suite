...
def compute_global_median():
    ...
    if RELAXED.exists():
        ...
        r['planned_duration_days'] = pd.to_numeric(r.get('planned_duration_days'), errors='coerce')
    ...
    if RULEB.exists():
        ...
        rb['planned_duration_days'] = pd.to_numeric(rb.get('planned_duration_days'), errors='coerce')
        vals = rb[rb.get('imputation_rule', '') != 'RuleB']['planned_duration_days'].dropna()
        vals = vals[(vals > 0) & (vals <= 1825)]
        if len(vals) > 0:
            return float(vals.median())
    # final fallback
...
def main():
    ...
    with open(OUT_DIAG, 'w', encoding='utf-8') as f:
        f.write(f'Global median planned_duration_days used (fallback logic): {global_median}\n')
