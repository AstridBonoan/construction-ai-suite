...
NON_DATE_TOKENS = set(
    ["unknown", "na", "n/a", "none", "ieh", "doer", "pns", "ftk", "does"]
)
...
def main():
    ...
    for _, prow in agg.iterrows():
        ...
        if pd.isna(a_start):
            ...
            # compare as dates
            match_found = any(
                (pd.Timestamp(a_start).normalize() == ds.normalize())
                for ds in parsed_starts
            )
            if not match_found:
                is_imputed = True
        if is_imputed:
            ...
            planned_dur = (
                float(prow["planned_duration_days"])
                if prow["planned_duration_days"] not in (None, "", "nan")
                else None
            )
            elapsed_days = (
                float(prow["elapsed_days"])
                if prow["elapsed_days"] not in (None, "", "nan")
                else None
            )
            delay_days = (
                float(prow["delay_days"])
                if prow["delay_days"] not in (None, "", "nan")
                else None
            )
            # differences
            ...
            if not pd.isna(a_start) and not pd.isna(planned_start):
                diff_actualstart_plannedstart = (
                    pd.Timestamp(a_start) - pd.Timestamp(planned_start)
                ).days
            if not pd.isna(a_start) and not pd.isna(planned_end):
                diff_actualstart_plannedend = (
                    pd.Timestamp(a_start) - pd.Timestamp(planned_end)
                ).days
            details.append(
                {
                    "project_id": pid,
    ...
    lines.append(
        "project_id, parsed_actual_start_candidates_count, actual_start, actual_end, planned_start, planned_end, planned_duration_days, elapsed_days, delay_days, diff_actualstart_plannedstart_days, diff_actualstart_plannedend_days"
    )
    for d in details:
        lines.append(
            ",".join(
                [
                    ...
                    for k in [
                        "project_id",
                        "parsed_actual_start_candidates_count",
                        "actual_start",
                        "actual_end",
                        "planned_start",
                        "planned_end",
                        ...
                        "elapsed_days",
                        "delay_days",
                        "diff_actualstart_plannedstart_days",
                        "diff_actualstart_plannedend_days",
                    ]
                ]
            )
        )
