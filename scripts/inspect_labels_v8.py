...
mask = (pd.to_numeric(df["delay_days"], errors="coerce") > 0) | (
    pd.to_numeric(df["schedule_slippage_pct"], errors="coerce") > 0.05
)
s = df.loc[
    mask,
    [
...
if "actual_start_imputed" in df.columns:
    imp = df[
        df["actual_start_imputed"].astype(str).str.lower().isin(["true", "1", "yes"])
    ][
        [
            "project_id",
            "planned_start",
