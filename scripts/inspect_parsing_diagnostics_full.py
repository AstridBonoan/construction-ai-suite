...
def main():
    ...
    for idx, row in agg.iterrows():
        ...
        if isinstance(fb, dict):
            ...
            rec["parse_fallback_used"] = (
                f"used_keys={list(used.keys()) if isinstance(used, dict) else used}; year_only={ro}; actual_start_imputed={fb.get('actual_start_imputed', False)}"
            )
        else:
        ...
        rec["flag_planned_start_eq_planned_end"] = bool(
            ps and pe and str(ps) == str(pe)
        )
        rec["flag_missing_actual_start"] = not (ast and str(ast).lower() != "nan")
        rec["flag_missing_planned_start_or_end"] = not (ps and pe)
        rec["flag_actual_end_present_but_start_missing"] = bool(
            aet and (not ast or str(ast).lower() == "nan")
        )
        rec["flag_actual_start_imputed"] = bool(
            str(rec.get("actual_start_imputed", False)).lower() in ("true", "1")
        )
