...
def main():
    ...
    for c in candidate_cols:
        ...
        report_lines.append(
            f"  non_null: {non_null_count} ({pct(non_null_count,total)}%)"
        )
        report_lines.append(
            f"  year_only (####): {year_only_count} ({pct(year_only_count,total)}%)"
        )
        report_lines.append(
            f"  full_date matches: {full_date_count} ({pct(full_date_count,total)}%)"
        )
    ...
    def per_project_counts(colname):
        ...
        return grp[colname].apply(
            lambda s: s.fillna("").astype(str).str.strip().ne("").sum()
        )
    ...
    for special in ["PermitYear", "CompltYear"]:
        if special in df.columns:
            ...
            report_lines.append(
                f"{special}: projects with >=1: {counts[special]['projects_with_at_least_one']} of {len(pc)}"
            )
        else:
    ...
    report_lines.append(
        "Top 5 example projects (project_id) with most date-like fields:"
    )
