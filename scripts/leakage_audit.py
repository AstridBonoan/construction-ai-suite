...
def analyze():
    ...
    for col in X.columns:
        ...
        rows.append({
            ...
            'abs_corr_with_will_delay': abs(corr_with_y) if not math.isnan(corr_with_y) else float('nan'),
            'corr_with_schedule_slippage_pct': corr_with_schedule,
            'abs_corr_with_schedule_slippage_pct': abs(corr_with_schedule) if not math.isnan(corr_with_schedule) else float('nan'),
        })

    dfres = pd.DataFrame(rows).sort_values(by='abs_corr_with_will_delay', ascending=False)
    ...
    if len(high_corr) > 0:
        ...
        for _, r in high_corr.head(20).iterrows():
            diag_lines.append(f" - {r['feature']}: corr_with_will_delay={r['corr_with_will_delay']:.4f}, constant={r['constant']}, id_like={r['id_like']}, date_like={r['date_like']}")

    # also check schedule correlations if present
    if schedule_col and schedule_col in X.columns:
        high_sched = dfres[dfres['abs_corr_with_schedule_slippage_pct'] >= 0.9]
        diag_lines.append(f'Features with |corr(schedule_slippage_pct)| >= 0.9: {len(high_sched)}')
        for _, r in high_sched.head(20).iterrows():
            diag_lines.append(f" - {r['feature']}: corr_with_schedule={r['corr_with_schedule_slippage_pct']:.4f}")

    # Suggest removals: id-like, date-like, constant, or very high corr
    suggestions = []
    for _, r in dfres.iterrows():
        if r['constant'] or r['id_like'] or r['date_like'] or r['abs_corr_with_will_delay'] >= 0.95 or r['abs_corr_with_schedule_slippage_pct'] >= 0.95:
    ...
    try:
        ...
        sns.heatmap(topn[['corr_with_will_delay', 'corr_with_schedule_slippage_pct']].astype(float).fillna(0).T, annot=True, cmap='coolwarm')
        ...
        sns.barplot(x='abs_corr_with_will_delay', y='feature', data=topn.head(20), palette='viridis')
