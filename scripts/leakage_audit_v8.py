...
def find_date_column(df: pd.DataFrame):
    candidates = ['planned_start_dt_parsed', 'planned_start_dt', 'planned_start', 'planned_end_dt_parsed', 'planned_end_dt']
    for c in candidates:
        if c in df.columns:
...
def leakage_audit():
    ...
    report = {'target': target, 'date_col': date_col, 'split_type': split_type, 'n_total': len(df), 'n_train': len(train), 'n_test': len(test)}
    ...
    if vals.empty:
        ...
        report['target_flags']['too_few_unique'] = int(vals.nunique()) < max(3, 0.01 * len(vals))
    ...
    # identifier overlap
    id_cols = [c for c in ['project_id', 'ID', 'Job_Number', 'Job_Number_x', 'Job_Number.y'] if c in df.columns]
    id_overlap = {}
    for c in id_cols:
        train_ids = set(train[c].dropna().astype(str))
        test_ids = set(test[c].dropna().astype(str))
        inter = train_ids.intersection(test_ids)
        id_overlap[c] = {'train_unique': len(train_ids), 'test_unique': len(test_ids), 'overlap_count': len(inter)}
    report['id_overlap'] = id_overlap

    # Target leakage via features
    # suspicious column name scan
    suspicious = []
    leak_keywords = ['delay', 'elapsed', 'slippage', 'actual_end', 'actual_start', 'planned_end', 'planned_start', 'imputed']
    for c in df.columns:
        lc = c.lower()
    ...
    if date_col:
        ...
        report['date_split'] = {
            ...
            'train_before_test': bool(pd.notna(train_max) and pd.notna(test_min) and train_max < test_min)
        }
    ...
    print('Target:', report['target'], 'std small?', report['target_flags'].get('std_too_small'), 'unique_count:', report['target_stats']['unique'])
    print('Duplicates between splits:', report['duplicates_between_splits'])
    if id_overlap:
        for k,v in id_overlap.items():
            print(f"ID col {k}: train_unique={v['train_unique']} test_unique={v['test_unique']} overlap={v['overlap_count']}")
    if suspicious:
