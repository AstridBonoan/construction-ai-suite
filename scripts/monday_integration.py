...
class MondayClient:
    ...
    def __init__(self, api_key: str, board_id: int, columns: Dict[str, str], cfg: dict = None, cfg_path: Path = None, encrypted: bool = False):
        self.api_key = api_key
        self.board_id = int(board_id) if board_id is not None else None
        self.columns = columns or {}
        self.headers = {"Authorization": api_key, "Content-Type": "application/json"} if api_key else None
    ...
    def run_query(self, query: str, variables: dict = None):
        ...
        if self.token_info:
            self.headers = {"Authorization": self.token_info.get('access_token'), "Content-Type": "application/json"}
        if not self.headers:
    ...
    def find_item_by_project_id(self, project_id: str) -> Optional[str]:
        ...
        '''
        variables = {'boardId': self.board_id, 'columnId': col, 'value': str(project_id)}
    ...
    def create_item(self, project_row: dict) -> Dict[str, Any]:
        ...
        '''
        resp = self.run_query(create_mutation, {'boardId': self.board_id, 'itemName': name})
        ...
        if column_values:
            ...
            '''
            self.run_query(change_mutation, {'itemId': int(item_id), 'boardId': int(self.board_id), 'columnVals': json.dumps(column_values)})

        return {'action': 'create', 'item_id': item_id}

    def update_item(self, item_id: str, project_row: dict) -> Dict[str, Any]:
        if DRY_RUN:
            return {'action': 'update', 'item_id': item_id, 'project_id': project_row.get('project_id')}
        self.ensure_token_valid()
        column_values = {}
        for key in ('predicted_delay', 'revenue', 'risk', 'status'):
            col_id = self.columns.get(key)
    ...
    def ensure_token_valid(self):
        ...
        if not refresh_token or not client_id or not client_secret:
            # cannot refresh
            self.api_key = token.get('access_token')
            self.headers = {"Authorization": self.api_key, "Content-Type": "application/json"} if self.api_key else None
            return
...
def find_item_by_project_id(project_id: str):
    ...
    if PROJECT_ID_COLUMN_ID.startswith("PROJECT_ID_COLUMN_PLACEHOLDER"):
        raise RuntimeError("Set PROJECT_ID_COLUMN_ID to your board's project id column ID before running live")
    ...
    '''
    variables = {'boardId': int(MONDAY_BOARD_ID), 'columnId': PROJECT_ID_COLUMN_ID, 'value': str(project_id)}
...
def create_item(project_row: dict):
    ...
    '''
    resp = run_query(create_mutation, {'boardId': int(MONDAY_BOARD_ID), 'itemName': name})
    item_id = resp.get('data', {}).get('create_item', {}).get('id')
    ...
    if any(k.endswith("PLACEHOLDER") for k in (COL_PREDICTED_DELAY, COL_REVENUE, COL_RISK, COL_STATUS)):
    ...
    '''
    run_query(change_mutation, {'itemId': int(item_id), 'boardId': int(MONDAY_BOARD_ID), 'columnVals': json.dumps(column_values)})
    return {'action': 'create', 'item_id': item_id}
...
def update_item(item_id: str, project_row: dict):
    ...
    if DRY_RUN:
        return {'action': 'update', 'item_id': item_id, 'project_id': project_row['project_id']}
    if any(k.endswith("PLACEHOLDER") for k in (COL_PREDICTED_DELAY, COL_REVENUE, COL_RISK, COL_STATUS)):
        raise RuntimeError("Replace column ID placeholders before running live")
...
def load_config(path: Path) -> dict:
    ...
    try:
        ...
        if isinstance(wrapper, dict) and wrapper.get('_encrypted'):
            ...
            if not secret:
                raise RuntimeError(f"Config {path} is encrypted but MONDAY_SECRET_KEY is not set")
            if Fernet is None:
                raise RuntimeError('cryptography package required to decrypt configs; pip install cryptography')
...
def save_config(path: Path, cfg: dict, encrypted: bool):
    try:
        ...
        if encrypted:
            secret = os.getenv('MONDAY_SECRET_KEY')
            if not secret:
                raise RuntimeError('MONDAY_SECRET_KEY not set; cannot save encrypted config')
            if Fernet is None:
                raise RuntimeError('cryptography package required to encrypt configs; pip install cryptography')
            f = Fernet(secret.encode())
...
def process_account_config(cfg: dict, cfg_name: str, input_csv: Path, report_dir: Path, cfg_path: Path = None, encrypted: bool = False):
    ...
    if 'project_id' not in columns:
        raise RuntimeError(f"Config {cfg_name} missing column mapping for 'project_id' (required).")

    client = MondayClient(api_key, board_id, columns, cfg=cfg, cfg_path=cfg_path, encrypted=encrypted)
    ...
    with open(input_csv, newline='', encoding='utf-8') as fh:
        ...
        for row in reader:
            ...
            if not project_id:
                report['failed'].append({'project_id': project_id, 'error': 'missing project_id'})
                continue
            try:
                ...
                if item_id:
                    ...
                    report['updated'] += 1
                    report['ops'].append({'action': 'update', 'project_id': project_id, 'item_id': item_id})
                else:
                    res = client.create_item(row)
                    report['created'] += 1
                    report['ops'].append({'action': 'create', 'project_id': project_id, 'item_id': res.get('item_id')})
                time.sleep(0.05)
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Run Monday.com integration for one or more account configs')
    parser.add_argument('--config', help='Path to a single account JSON config')
    parser.add_argument('--configs-dir', help='Directory containing per-account JSON configs')
    parser.add_argument('--input-csv', help='Input CSV path', default=str(INPUT_CSV))
    parser.add_argument('--report-dir', help='Directory to write per-account reports', default='reports')
    ...
    if args.configs_dir:
        ...
        if not p.exists() or not p.is_dir():
            ...
            return
        configs.extend(sorted([x for x in p.iterdir() if x.suffix.lower() in ('.json',)]))
    if MONDAY_CONFIG and not configs:
        configs.append(Path(MONDAY_CONFIG))

    if not configs:
        print('No config provided. Use --config or --configs-dir, or set MONDAY_CONFIG env var.')
        return
    ...
    for cfg_path in configs:
        ...
        try:
            ...
            out_path, report = process_account_config(cfg, cfg_name, input_csv, report_dir, cfg_path=cfg_path, encrypted=encrypted)
            logger.info(f"Wrote report {out_path} - created {report['created']}, updated {report['updated']}, failed {len(report['failed'])}")
        except Exception as e:
...
def readiness_check():
    ...
    if DRY_RUN:
        ...
        logger.info("DRY_RUN disabled — script will attempt live API calls if credentials and column IDs are set")

    placeholders = [PROJECT_ID_COLUMN_ID, COL_PREDICTED_DELAY, COL_REVENUE, COL_RISK, COL_STATUS]
    if any(p.endswith("PLACEHOLDER") for p in placeholders):
        print("NOTE: Some column ID placeholders are still present — replace them before running live")

    if ok:
        logger.info("Integration scripts ready.")
        logger.info("Enter your MONDAY_API_KEY, MONDAY_BOARD_ID, and column IDs in monday_integration.py placeholders before live run.")
