...
proj_primary = sum(1 for rec in js if any('project phase' in c.lower() for pcs in rec['used'].values() for c in pcs))
proj_fallback = sum(1 for rec in js if any(not ('project phase' in c.lower()) for pcs in rec['used'].values() for c in pcs))
