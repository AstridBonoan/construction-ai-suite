...
OUT_DIAG = ROOT / 'data_splits' / 'v8' / 'diagnostics_global_median_and_missing_type.txt'
...
def main():
    ...
    percentiles = np.percentile(vals, [0, 1, 5, 10, 25, 50, 75, 90, 95, 99, 100]) if len(vals) > 0 else []
    ...
    try:
        ...
        if len(vals) > 0:
            ...
            plt.axvline(global_median, color='red', linestyle='--', label=f'median={global_median:.1f}')
    ...
    if pt_col and pt_col in relaxed.columns:
        missing_mask = relaxed[pt_col].isna() | (relaxed[pt_col].astype(str).str.strip() == '')
    else:
        missing_mask = pd.Series([True] * len(relaxed), index=relaxed.index)

    missing = relaxed.loc[missing_mask, [id_col, 'planned_start', 'planned_end', 'actual_start', 'actual_end', 'actual_start_imputed', 'imputation_rule']].copy()
    ...
    if RULEB.exists():
        ...
        total_imputed = int((relaxed.get('imputation_rule', '') == 'RuleB').sum()) if 'imputation_rule' in relaxed.columns else 0
        counts_by_rule = relaxed.get('imputation_rule', pd.Series()).fillna('none').value_counts().to_dict()
        counts_by_conf = relaxed.get('label_confidence', pd.Series()).fillna('low').value_counts().to_dict()

    missing_count = int(missing.shape[0])

    # write diagnostics text
    with open(OUT_DIAG, 'w', encoding='utf-8') as f:
        f.write(f'Global median planned_duration_days (from relaxed, >0): {global_median}\n')
        ...
        for k, v in counts_by_conf.items():
            f.write(f' - {k}: {int(v)}\n')
        f.write('\nPlotted histogram saved to: {}\n'.format(OUT_PLOT if plotted else '<not created>'))
    ...
    if len(percentiles) > 0:
        print('Percentiles (0,1,5,10,25,50,75,90,95,99,100):', ','.join([f'{p:.2f}' for p in percentiles]))
    if plotted:
