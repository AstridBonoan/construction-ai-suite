...
def load_audit_flags():
    ...
    for _, r in df.iterrows():
        ...
        try:
            if (
                not pd.isna(r.get("abs_corr_with_will_delay"))
                and float(r.get("abs_corr_with_will_delay")) >= 0.95
            ):
                dropc.add(r["feature"])
        except Exception:
            pass
        try:
            if (
                not pd.isna(r.get("abs_corr_with_schedule_slippage_pct"))
                and float(r.get("abs_corr_with_schedule_slippage_pct")) >= 0.95
            ):
                dropc.add(r["feature"])
        except Exception:
            pass
...
def prepare(random_state: int = 42, test_size: float = 0.2):
    ...
    if "project_id" not in df.columns:
        ...
        df["project_id"] = (
            ...
            + df.get("Project School Name", "").fillna("")
        ).factorize()[0]
    ...
    meta = {
        ...
        "n_projects_test": (
            int(len(test_projects)) if isinstance(test_projects, (set, list)) else 0
        ),
