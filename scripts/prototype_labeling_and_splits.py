...
def main():
    ...
    for col in ["will_delay_opt1", "will_delay_opt2"]:
        ...
        print(
            f"Counts for {col}: positives={counts[col][0]} negatives={counts[col][1]}"
        )
    ...
    def make_splits(label_col, prefix):
        ...
        if total_pos == 0 or total_pos == total:
            print(
                f"Skipping splits for {label_col}: single-class (pos={total_pos} of {total})"
            )
            return False
        ...
        y_train.to_csv(
            os.path.join(folder, "y_train.csv"), index=False, header=[label_col]
        )
        y_val.to_csv(os.path.join(folder, "y_val.csv"), index=False, header=[label_col])
        y_test.to_csv(
            os.path.join(folder, "y_test.csv"), index=False, header=[label_col]
        )
