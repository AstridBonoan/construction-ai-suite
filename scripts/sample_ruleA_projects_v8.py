...
def main():
    ...
    missing_actual_start = int(
        sample["actual_start"].isna().sum()
        if "actual_start" in sample.columns
        else sum(1 for v in sample["actual_start"] if str(v).strip() == "")
    ).__int__()
    missing_actual_end = int(
        sample["actual_end"].isna().sum()
        if "actual_end" in sample.columns
        else sum(1 for v in sample["actual_end"] if str(v).strip() == "")
    ).__int__()
