#!/usr/bin/env python3
"""Train Baseline v5 on cleaned v5 splits.

Saves model, metrics, and plots under models/v5 and analysis_outputs/v5
"""
...
OUT_MODEL = ROOT / "models" / "v5" / "baseline_project_delay_model_v5.pkl"
OUT_METRICS = ROOT / "analysis_outputs" / "v5" / "metrics.json"
OUT_PLOTS = ROOT / "analysis_outputs" / "v5"
...
def train_v5(random_state: int = 42, n_iter: int = 12):
    ...
    if HAVE_PLOT:
        try:
            ...
            plt.title("v5 Actual vs Predicted")
            plt.tight_layout()
            ...
            plt.xlabel("Residual")
            plt.title("v5 Residuals Distribution")
            plt.tight_layout()
            ...
            plt.title("v5 Absolute Error Histogram")
            plt.tight_layout()
    ...
    print("Saved v5 model to", OUT_MODEL)
    print("Saved v5 metrics to", OUT_METRICS)
    print("Saved v5 plots to", OUT_PLOTS)
...
if __name__ == "__main__":
    ...
    train_v5(random_state=args.random_state, n_iter=args.n_iter)
