...
def build_pipeline(X: pd.DataFrame):
    ...
    cat_pipeline = Pipeline([('imputer', SimpleImputer(strategy='constant', fill_value='__MISSING__')), ('ohe', OneHotEncoder(handle_unknown='ignore'))])

    pre = ColumnTransformer([('num', num_pipeline, numeric), ('cat', cat_pipeline, categorical)])

    pipe = Pipeline([('pre', pre), ('rf', RandomForestRegressor(n_jobs=-1, random_state=42))])
    return pipe
...
def train_and_save():
    ...
    search = RandomizedSearchCV(pipe, param_distributions=param_dist, n_iter=8, cv=3, n_jobs=1, random_state=42)
