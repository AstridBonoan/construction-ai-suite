...
parser.add_argument(
    "--include-imputed",
    action="store_true",
    help="Include rows with actual_start_imputed=True in training",
)
parser.add_argument(
    "--label",
    type=str,
    default="will_delay",
    help="Label to use (will_delay or will_delay_abs30)",
)
...
if not INCLUDE_IMPUTED:
    ...
    if imputed_col in X_train.columns:
        exclude_mask_train = (
            X_train[imputed_col].astype(str).str.lower().isin(["true", "1", "yes"])
        )
    if imputed_col in X_test.columns:
        exclude_mask_test = (
            X_test[imputed_col].astype(str).str.lower().isin(["true", "1", "yes"])
        )
...
if (
    ...
    with open(MODEL_CARD, "w") as f:
        ...
        if INCLUDE_IMPUTED:
            f.write(
                "Note: actual_start_imputed rows WERE INCLUDED in training (flag present as a feature if available).\n"
            )
        else:
    ...
    with open(MODEL_CARD, "w") as f:
        ...
        if INCLUDE_IMPUTED:
            f.write(
                "Imputation: projects with `actual_start_imputed=True` WERE INCLUDED in training and the flag was retained as a feature where present.\n"
            )
        else:
            f.write(
                "Imputation: projects with `actual_start_imputed=True` were excluded from training.\n"
            )
        f.write("\n")
        ...
        f.write(
            json.dumps(
                {"accuracy": acc, "precision": prec, "recall": rec, "f1": f1}, indent=2
            )
            + "\n"
        )
