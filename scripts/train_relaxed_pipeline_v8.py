...
def main():
    ...
    # Imputation flags
    agg['actual_start_imputed'] = agg.get('actual_start_imputed', False).astype(str).fillna('False')
    agg['imputation_rule'] = agg.get('imputation_rule', 'none').fillna('none')

    # Assign label_confidence
    def assign_confidence(row):
        imputed = str(row['actual_start_imputed']).strip().lower() in ('1','true','t','yes')
    ...
    if 'will_delay' in computable.columns:
        computable['will_delay'] = pd.to_numeric(computable['will_delay'], errors='coerce').fillna(0).astype(int)
    else:
        computable['will_delay'] = (computable['delay_days'] > 0).astype(int)

    y = computable['will_delay']
    X = computable.drop(columns=['will_delay', 'will_delay_abs30', 'will_delay_rel5pct'], errors=True)

    # Save full computable CSV for reference
    computable.to_csv(OUT_DIR / 'project_level_aggregated_v8_relaxed_computable.csv', index=False)
    ...
    if len(comp_filtered) >= 1:
        ...
        Xf = comp_filtered.drop(columns=['will_delay', 'will_delay_abs30', 'will_delay_rel5pct'], errors=True)
        if len(Xf) < 2:
        ...
        yf_train.to_frame('will_delay').to_csv(OUT_DIR / 'y_train_relaxed_filtered.csv', index=False)
        yf_test.to_frame('will_delay').to_csv(OUT_DIR / 'y_test_relaxed_filtered.csv', index=False)

    # Training: basic RandomForest on numeric columns
    # Drop deny-list columns
    DENY_LIST = set([c.lower() for c in (['will_delay', 'schedule_slippage_pct', 'award', 'bbl', 'bin', 'cost', 'cost_estimated'])])
    ...
    metrics = {'train_counts': train_counts, 'test_counts': test_counts, 'counts_by_confidence': counts_conf}
    ...
    if len(train_counts) <= 1 or sum(v for k,v in train_counts.items() if str(k) in ('1','True')) == 0:
        metrics['warning'] = 'Single-class or no positive examples in training set; skipping model training.'
